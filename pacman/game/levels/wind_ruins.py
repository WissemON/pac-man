...
class WindRuins(Level):
    """Define the wind ruins themed level."""

    def __init__(self, game: "Game", maze_grid: list[list[int]]) -> None:
        """Initialize the wind ruins.
        ...
        self.bgm_intro = resource_path(
            'music/levels/level_07/wind_ruins_intro.mp3')
        self.bgm_loop = resource_path(
            'music/levels/level_07/wind_ruins_loop.mp3')
        self.tiles = SpriteSheet(resource_path(
            'levels/level_7', 'tiles1.png'), self.game)
        self.tiles2 = SpriteSheet(resource_path(
            'levels/level_6', 'castor_wilds2.png'), self.game)
        sheet = SpriteSheet(resource_path('items', 'items.png'), self.game)
        self.special_item = sheet.get_sprite(61, 236, 16, 16, (192, 192, 255))

        self.lonlon = SpriteSheet(resource_path(
            'levels/level_1', 'lon_lon_ranch.png'), self.game)
        self.bridge = SpriteSheet(resource_path(
            'levels/level_2', 'bridge.png'), self.game)

    def _load_default_enemy_skins(self) -> None:
        """Load the default enemy sprite animations."""
        enemy_sheet = SpriteSheet(resource_path(
            'enemies', 'ropes.png'), self.game)

        self.blinky = {
            'up': [
                enemy_sheet.get_sprite(1, 67, 14, 14, (0, 0, 0)),
            ],
            'down': [
                enemy_sheet.get_sprite(2, 44, 14, 15, (0, 0, 0)),
            ],
            'left': [
                enemy_sheet.get_sprite(1, 0, 15, 16, (0, 0, 0)),
            ],
            'right': [
                enemy_sheet.get_sprite(0, 23, 15, 16, (0, 0, 0)),
            ]
        }

        self.clyde = {
            'up': [
                enemy_sheet.get_sprite(1, 316, 14, 14, (0, 0, 0)),
            ],
            'down': [
                enemy_sheet.get_sprite(2, 293, 14, 15, (0, 0, 0)),
            ],
            'left': [
                enemy_sheet.get_sprite(1, 249, 15, 16, (0, 0, 0)),
            ],
            'right': [
                enemy_sheet.get_sprite(0, 272, 15, 16, (0, 0, 0)),
            ]
        }

        self.inky = {
            'up': [
                enemy_sheet.get_sprite(1, 233, 14, 14, (0, 0, 0)),
            ],
            'down': [
                enemy_sheet.get_sprite(2, 210, 14, 15, (0, 0, 0)),
            ],
            'left': [
                enemy_sheet.get_sprite(1, 166, 15, 16, (0, 0, 0)),
            ],
            'right': [
                enemy_sheet.get_sprite(0, 189, 15, 16, (0, 0, 0)),
            ]
        }
    ...
    def spawn_random_item(self) -> None:
        ...
        if free_points:
            ...
            SpecialItem(self.game, spawn_x, spawn_y, 61, 236, 16, 16)
    ...
    def _setup_maze_theme_assets(self) -> None:
        ...
        self.biomes_config = {
            'north_west': {
                ...
                'ground_base': self.tiles.get_sprite(96, 320, 16, 16),
                'wall_variants': [
                    self.tiles.get_sprite(16, 48, 16, 16),
                ],
                'ground_variants': [
                    self.tiles.get_sprite(96, 320, 16, 16),
                    self.tiles.get_sprite(96, 336, 16, 16),
                    self.tiles.get_sprite(112, 336, 16, 16),
                ]
            },
            'north_east': {
                'wall_base': self.tiles.get_sprite(192, 944, 16, 16),
                'ground_base': self.tiles.get_sprite(0, 912, 16, 16),
                'wall_variants': [
                    self.tiles.get_sprite(192, 944, 16, 16),
                    self.tiles.get_sprite(208, 944, 16, 16),
                ],
                'ground_variants': [
                    self.tiles.get_sprite(0, 912, 16, 16),
                    self.tiles.get_sprite(0, 928, 16, 16),
                    self.tiles.get_sprite(0, 944, 16, 16),
                    self.tiles.get_sprite(0, 960, 16, 16),
                ]
            },
            'south_west': {
                'wall_base': self.tiles.get_sprite(16, 32, 16, 16),
                'ground_base': self.tiles.get_sprite(0, 32, 16, 16),
                'wall_variants': [
                    self.tiles.get_sprite(16, 32, 16, 16),
                    self.tiles.get_sprite(64, 80, 16, 16),
                ],
                'ground_variants': [
                    self.tiles.get_sprite(0, 32, 16, 16),
                    self.tiles.get_sprite(0, 48, 16, 16),
                ]
            },
            'south_east': {
                'wall_base': self.tiles.get_sprite(416, 169, 16, 16),
                'ground_base': self.tiles.get_sprite(128, 848, 16, 16),
                'wall_variants': [
                    self.tiles.get_sprite(416, 1696, 16, 16),
                    self.tiles.get_sprite(432, 1696, 16, 16),
                ],
                'ground_variants': [
                    self.tiles.get_sprite(128, 848, 16, 16),
                    self.tiles.get_sprite(128, 864, 16, 16),
                    self.tiles.get_sprite(144, 848, 16, 16),
                    self.tiles.get_sprite(144, 864, 16, 16),
                ]
            }
        }
    ...
    def generate_maze(self) -> None:
        ...
        for y, maze_row in enumerate(self.maze_grid):
            for x, cell in enumerate(maze_row):
                ...
                Wall(self.game, new_x, new_y,
                     base_image=assets['wall_base'],
                     variants=assets['wall_variants'])
                Wall(self.game, new_x + TILE_SIZE * 2, new_y + TILE_SIZE * 2,
                     base_image=assets['wall_base'],
                     variants=assets['wall_variants'])
                Wall(self.game, new_x + TILE_SIZE * 2, new_y,
                     base_image=assets['wall_base'],
                     variants=assets['wall_variants'])
                Wall(self.game, new_x, new_y + TILE_SIZE * 2,
                     base_image=assets['wall_base'],
                     variants=assets['wall_variants'])
        ...
        water_sprite = self.tiles.get_sprite(144, 224, 16, 16)
        bridge_sprite = self.tiles.get_sprite(144, 240, 16, 16)
    ...
    def setup_decorations(self) -> None:
        ...
        little_tree = self.tiles.get_sprite(96, 832, 16, 16)
        for c in range(0, cols + 5, 2):
            x = self.game.maze_offset_x + c * step + step
            DecorationUpDown(self.game, x, self.game.maze_offset_y -
                             random.randint(25, 45), little_tree)
            DecorationUpDown(
                self.game,
                x,
                self.game.maze_offset_y +
                self.game.maze_height +
                random.randint(
                    3,
                    28),
                little_tree)
    ...
    def fill_background(self, surface: pygame.Surface) -> None:
        ...
        """
        floor = self.tiles.get_sprite(112, 352, 16, 16)
    ...
    def render_special(self, surface: pygame.Surface) -> None:
        ...
        Args:
            surface: Surface that receives drawing operations.
        """
        top_left_corner = self.tiles.get_sprite(256, 1808, 16, 16)
        top_middle = self.tiles.get_sprite(272, 1808, 16, 16)
        top_right_corner = self.tiles.get_sprite(288, 1808, 16, 16)
        left_middle = self.tiles.get_sprite(256, 1824, 16, 16)
        bottom_left_corner = self.tiles.get_sprite(256, 1840, 16, 16)
        bottom_middle = self.tiles.get_sprite(272, 1840, 16, 16)
        bottom_right_corner = self.tiles.get_sprite(288, 1840, 16, 16)
        right_middle = self.tiles.get_sprite(288, 1824, 16, 16)

        corner_left_to_bottom = self.tiles.get_sprite(272, 1920, 16, 16)
        corner_bottom_to_left = self.tiles.get_sprite(272, 1920, 16, 16)
        ...
        middle_01 = self.tiles.get_sprite(272, 1824, 16, 16)
        middle_02 = self.tiles.get_sprite(272, 1920, 16, 16)
