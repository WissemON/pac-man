...
class VaatiBattle(Level):
    """Define the vaati battle themed level."""

    def __init__(self, game: "Game", maze_grid: list[list[int]]) -> None:
        """Initialize the vaati battle.
        ...
        self.item_spawn_delay = 10000
        self.item_spawn_timer = 0
        self.item_spawned = False
        self.bgm_loop = resource_path(
            'music/levels/level_15/vaati_battle_loop.mp3')
        self.tiles1 = SpriteSheet(resource_path(
            'levels/level_15', 'tiles.png'), self.game)
        self.tiles2 = SpriteSheet(resource_path(
            'levels/level_15', 'tiles2.png'), self.game)
        self.vaati = SpriteSheet(resource_path(
            'levels/level_15', 'vaati.png'), self.game)
        item_sheet = SpriteSheet(resource_path(
            'items', 'items.png'), self.game)
        self.special_item = item_sheet.get_sprite(
            4, 256, 16, 16, (192, 192, 255))
    ...
    def _load_default_enemy_skins(self) -> None:
        ...
        enemy_sheet = SpriteSheet(resource_path(
            'enemies', 'vaati.png'), self.game)

        left_b = enemy_sheet.get_sprite(18, 1, 17, 27, (0, 64, 128))
        right_b = pygame.transform.flip(left_b, True, False)
        self.blinky = {
            'up': [
                enemy_sheet.get_sprite(0, 2, 18, 26, (0, 64, 128)),
            ],
            'down': [
                enemy_sheet.get_sprite(34, 0, 18, 28, (0, 64, 128)),
            ],
            'left': [
                left_b,
            ],
        ...
        left_c = enemy_sheet.get_sprite(18, 57, 17, 27, (0, 64, 128))
        right_c = pygame.transform.flip(left_c, True, False)
        self.clyde = {
            'up': [
                enemy_sheet.get_sprite(0, 58, 18, 26, (0, 64, 128)),
            ],
            'down': [
                enemy_sheet.get_sprite(34, 56, 18, 28, (0, 64, 128)),
            ],
            'left': [
                left_c,
            ],
        ...
        left_i = enemy_sheet.get_sprite(18, 29, 16, 27, (0, 64, 128))
        right_i = pygame.transform.flip(left_i, True, False)
        self.inky = {
            'up': [
                enemy_sheet.get_sprite(0, 30, 18, 26, (0, 64, 128)),
            ],
            'down': [
                enemy_sheet.get_sprite(34, 28, 18, 28, (0, 64, 128)),
            ],
            'left': [
                left_i,
            ],
        ...
        left_p = enemy_sheet.get_sprite(18, 85, 16, 27, (0, 64, 128))
        right_p = pygame.transform.flip(left_p, True, False)
        self.pinky = {
            'up': [
                enemy_sheet.get_sprite(0, 86, 17, 26, (0, 64, 128)),
            ],
            'down': [
                enemy_sheet.get_sprite(34, 84, 18, 28, (0, 64, 128)),
            ],
            'left': [
                left_p,
            ],
    ...
    def spawn_random_item(self) -> None:
        ...
        if free_points:
            ...
            SpecialItem(self.game, spawn_x, spawn_y, 4, 256, 16, 16)
    ...
    def _setup_maze_theme_assets(self) -> None:
        ...
        self.biomes_config = {
            'north_west': {
                'wall_base': self.tiles1.get_sprite(224, 0, 16, 16),
                'ground_base': self.tiles1.get_sprite(272, 1136, 16, 16),
                'wall_variants': [
                    self.tiles1.get_sprite(224, 0, 16, 16),
                ],
                'ground_variants': [
                    self.tiles1.get_sprite(272, 1136, 16, 16),
                ]
            },
            'north_east': {
                'wall_base': self.tiles1.get_sprite(224, 0, 16, 16),
    ...
    def generate_maze(self) -> None:
        ...
        self.game.player = Player(self.game, player_x, player_y, 'yellow')
        ...
        water_sprite = self.tiles1.get_sprite(128, 16, 16, 16)
        bridge_sprite = self.tiles1.get_sprite(128, 0, 16, 16)
        ...
        Water(self.game, x + 16, walk_y - TILE_SIZE,
              self.tiles1.get_sprite(128, 16, 16, 16))
        ...
        Water(self.game, self.game.maze_offset_x - 48,
              walk_y - TILE_SIZE, self.tiles1.get_sprite(128, 16, 16, 16))
        Bridge(self.game, self.game.maze_offset_x - 48, walk_y,
               self.tiles1.get_sprite(112, 624, 16, 16))
        Water(self.game, self.game.maze_offset_x - 48,
              walk_y + TILE_SIZE, self.tiles1.get_sprite(128, 16, 16, 16))

        Water(self.game, -16, walk_y - TILE_SIZE,
              self.tiles1.get_sprite(128, 16, 16, 16))
    ...
    def setup_decorations(self) -> None:
        ...
        dungeon_wall_up = self.vaati.get_sprite(134, 297, 46, 61, (0, 40, 81))
        x = self.game.maze_offset_x + middle * 48
        DecorationBorder(self.game, x - 48 - 48,
    ...
    def setup_gums(self, nb_pac_gums: int) -> None:
        ...
        for x, y in corners:
            if (x, y) in reserved:
                world_x, world_y = self._grid_to_world_center(x, y)
                SuperPacGum(self.game, world_x, world_y, 73, 194, 16, 16)
    ...
    def fill_background(self, surface: pygame.Surface) -> None:
        ...
        """
        total_w = len(self.maze_grid[0]) * TILE_SIZE * 2 + 800
        total_h = len(self.maze_grid) * TILE_SIZE * 2 + 800

        floor = self.tiles2.get_sprite(32, 32, 240, 144)
