...
from pacman.game.entities import (
    Wall, Ground, DecorationBorder, DecorationUpDown, Player
)
from pacman.game.items import SuperPacGum, SpecialItem, PacGumRupee
...
class HyruleField(Level):
    """Define the hyrule field themed level."""

    def __init__(self, game: "Game", maze_grid: list[list[int]]) -> None:
        """Initialize the hyrule field.
    ...
    def spawn_random_item(self) -> None:
        ...
        if free_points:
            ...
            SpecialItem(self.game, spawn_x, spawn_y, 4, 215, 16, 16)
    ...
    def _setup_maze_theme_assets(self) -> None:
        ...
        self.biomes_config = {
            'north_west': {
                'wall_base': self.hf.get_sprite(16, 32, 16, 16),
                'ground_base': self.hf.get_sprite(192, 320, 16, 16),
                'wall_variants': [
                    self.hf.get_sprite(16, 32, 16, 16),
                    self.hf.get_sprite(32, 64, 16, 16),
                    self.hf.get_sprite(0, 64, 16, 16)
                ],
                'ground_variants': [
                    self.hf.get_sprite(192, 320, 16, 16),
                    self.hf.get_sprite(192, 336, 16, 16),
                    self.hf.get_sprite(192, 272, 16, 16),
                ]
            },
            'north_east': {
                'wall_base': self.hf.get_sprite(0, 128, 16, 16),
                'ground_base': self.hf.get_sprite(0, 272, 16, 16),
                'wall_variants': [
                    self.hf.get_sprite(16, 128, 16, 16),
                    self.hf.get_sprite(32, 128, 16, 16),
                    self.hf.get_sprite(48, 128, 16, 16)
                ],
                'ground_variants': [
                    self.hf.get_sprite(224, 288, 16, 16),
                    self.hf.get_sprite(224, 304, 16, 16),
                    self.hf.get_sprite(224, 320, 16, 16),
                ]
            },
            'south_west': {
                'wall_base': self.hf.get_sprite(16, 48, 16, 16),
                'ground_base': self.hf.get_sprite(0, 48, 16, 16),
                'wall_variants': [
                    self.hf.get_sprite(16, 96, 16, 16),
                    self.hf.get_sprite(16, 48, 16, 16)
                ],
                'ground_variants': [
                    self.hf.get_sprite(128, 256, 16, 16),
                    self.hf.get_sprite(128, 288, 16, 16),
                    self.hf.get_sprite(192, 272, 16, 16),
                    self.hf.get_sprite(128, 304, 16, 16),
                    self.hf.get_sprite(128, 272, 16, 16)
                ]
            },
            'south_east': {
                'wall_base': self.hf.get_sprite(64, 128, 16, 16),
                'ground_base': self.hf.get_sprite(160, 512, 16, 16),
                'wall_variants': [
                    self.hf.get_sprite(64, 144, 16, 16),
                    self.hf.get_sprite(64, 128, 16, 16)
                ],
                'ground_variants': [
                    self.hf.get_sprite(144, 736, 16, 16),
                    self.hf.get_sprite(144, 752, 16, 16),
                    self.hf.get_sprite(160, 736, 16, 16),
                    self.hf.get_sprite(160, 752, 16, 16),
                ]
            }
        }
    ...
    def generate_maze(self) -> None:
            ...
            for x, cell in enumerate(maze_row):
                ...
                if cell & 2:
                    if not is_right_tunnel:
                        Wall(self.game,
                             new_x + TILE_SIZE * 2, new_y + TILE_SIZE,
                             base_image=assets['wall_base'],
                             variants=assets['wall_variants'])
                else:
        ...
        water_sprite = self.lonlon.get_sprite(643, 513, 16, 16)
        bridge_sprite = self.bridge.get_sprite(0, 0, 16, 16)
    ...
    def setup_decorations(self) -> None:
        ...
        step = TILE_SIZE * 2 - 7
        misc = SpriteSheet(resource_path(
            'levels/level_1', 'miscs.png'), self.game)
        tree_img = misc.get_sprite(161, 165, 64, 65, (255, 255, 255))

        left_x = self.game.maze_offset_x - 45
        right_x = self.game.maze_offset_x + self.game.maze_width + 45
        for r in range(-16, rows + 7, 2):
            y = self.game.maze_offset_y + r * step + step
            if not self._rect_overlaps_tunnel(left_x, y, margin=120):
                DecorationBorder(self.game, left_x, y, tree_img)
            if not self._rect_overlaps_tunnel(right_x, y, margin=120):
                DecorationBorder(self.game, right_x, y, tree_img)

        flowers = self.game.images['flowers']
        for c in range(0, cols + 3, 2):
            x = self.game.maze_offset_x + c * step + step
            DecorationUpDown(
                self.game, x, self.game.maze_offset_y - random.randint(25, 45),
                flowers)
            DecorationUpDown(self.game, x, self.game.maze_offset_y +
                             self.game.maze_height + random.randint(3, 28),
                             flowers)
    ...
    def setup_gums(self, nb_pac_gums: int) -> None:
        ...
        for x, y in corners:
            if (x, y) in reserved:
                world_x, world_y = self._grid_to_world_center(x, y)
                SuperPacGum(self.game, world_x, world_y, 5, 194, 16, 16)
    ...
    def fill_background(self, surface: pygame.Surface) -> None:
        ...
        """
        floor_tile = self.game.images['floor_01']
