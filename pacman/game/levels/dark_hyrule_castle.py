...
class DarkHyruleCastle(Level):
    """Define the dark hyrule castle themed level."""

    def __init__(self, game: "Game", maze_grid: list[list[int]]) -> None:
        """Initialize the dark hyrule castle.
    ...
    def spawn_random_item(self) -> None:
        ...
        if free_points:
            ...
            SpecialItem(self.game, spawn_x, spawn_y, 409, 254, 16, 16)
    ...
    def _setup_maze_theme_assets(self) -> None:
        ...
        self.biomes_config = {
            'north_west': {
                'wall_base': self.tiles1.get_sprite(272, 1136, 16, 16),
            ...
            'south_west': {
                ...
                'ground_base': self.tiles1.get_sprite(64, 32, 16, 16),
                'wall_variants': [
                    self.tiles1.get_sprite(80, 80, 16, 16),
                ],
                'ground_variants': [
                    self.tiles1.get_sprite(64, 32, 16, 16),
                ]
            },
            'south_east': {
                'wall_base': self.tiles1.get_sprite(448, 32, 16, 16),
                'ground_base': self.tiles1.get_sprite(384, 64, 16, 16),
                'wall_variants': [
                    self.tiles1.get_sprite(448, 32, 16, 16),
                    self.tiles1.get_sprite(432, 32, 16, 16),
                ],
                'ground_variants': [
                    self.tiles1.get_sprite(384, 64, 16, 16),
                    self.tiles1.get_sprite(400, 64, 16, 16),
                    self.tiles1.get_sprite(416, 48, 16, 16),
                ]
            }
        }
    ...
    def generate_maze(self) -> None:
        ...
        Water(self.game, x + 16, walk_y - TILE_SIZE,
              self.tiles1.get_sprite(128, 608, 16, 16))
        Bridge(self.game, x + 16, walk_y,
               self.tiles1.get_sprite(128, 624, 16, 16))
        Water(self.game, x + 16, walk_y + TILE_SIZE,
              self.tiles1.get_sprite(128, 608, 16, 16))
    ...
    def setup_decorations(self) -> None:
        ...
        step = TILE_SIZE * 2 - 7
        dungeon_wall_l = self.tiles1.get_sprite(128, 656, 32, 48)
        dungeon_wall_r = self.tiles1.get_sprite(128, 704, 32, 48)
        ...
        dungeon_wall_up = self.tiles1.get_sprite(320, 272, 64, 32)
        dungeon_wall_down = self.tiles1.get_sprite(304, 688, 48, 32)
        for c in range(0, cols + 7, 1):
            x = self.game.maze_offset_x + c * 48
            DecorationBorder(self.game, x - 48,
                             self.game.maze_offset_y - 16, dungeon_wall_up)
            DecorationBorder(self.game, x - 48, self.game.maze_offset_y +
