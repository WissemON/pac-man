import pygame
from typing import TYPE_CHECKING, TypedDict
from pacman.game.scene_manager import Scene
from pacman.resources import resource_path
from pacman.game.sprites import SpriteSheet, StringFont
from pacman.highscores import load_highscores
from pacman.highscores import insert_highscore, save_highscores

if TYPE_CHECKING:
    from pacman.game.game import Game


class TimelineStep(TypedDict):
    """Describe one timed step in the ending timeline."""
    duration: float
    next: str


class WeirdEndingScene(Scene):
    """Display the alternate ending sequence."""

    def __init__(self, game: "Game") -> None:
        """Initialize the weird ending scene.

        Args:
            game: Shared game object that owns resources, groups, and state.
        """
        super().__init__(game)
        self.state = "Replay"
        self.score = False
        self.time = 0.0
        self.frame_index = 0
        self.frame_delay = 120
        self.zelda_sheet = SpriteSheet(resource_path("player", "zelda2.png"),
                                       game)
        self.link_s = SpriteSheet(
            resource_path("player", "minish_cap_sprites.png"),
            game)

        link_anim = [
            self.link_s.get_sprite(15, 461, 18, 24),
            self.link_s.get_sprite(46, 460, 19, 24),
            self.link_s.get_sprite(78, 461, 22, 23),
            self.link_s.get_sprite(109, 460, 22, 24),
            self.link_s.get_sprite(143, 461, 19, 23),
            self.link_s.get_sprite(175, 462, 19, 22),
            self.link_s.get_sprite(204, 462, 22, 22),
            self.link_s.get_sprite(237, 461, 20, 23),
        ]
        self.link_anim = [
            pygame.transform.scale(sprite, (sprite.get_width() * 3,
                                            sprite.get_height() * 3))
            for sprite in link_anim
        ]

        zelda = self.zelda_sheet.get_sprite(15, 712, 18, 23, (16, 128, 88))
        self.zelda = pygame.transform.scale(zelda, (zelda.get_width() * 3,
                                                    zelda.get_height() * 3))

        self.phase = 'win_screen'
        self.house_sheet = SpriteSheet(
            resource_path("player", "link_house.png"), game)
        self.link_sheet = SpriteSheet(
            resource_path("player", "minish_cap_sprites.png"), game)
        self.bubble_sheet = SpriteSheet(
            resource_path("player", "sleep.png"), game)
        self.house = self.house_sheet.get_sprite(40, 11, 240, 160)
        self.house = pygame.transform.scale(
            self.house, (
                self.game.logic_width, self.game.logic_height - 160))
        sleep = [
            self.link_sheet.get_sprite(144, 3051, 16, 15),
            self.link_sheet.get_sprite(176, 3051, 16, 15),
        ]
        self.sleep_s = [
            pygame.transform.scale(
                sprite, (sprite.get_width() * 4,
                         sprite.get_height() * 4)) for sprite in sleep
        ]
        awake = [
            self.link_sheet.get_sprite(240, 3053, 16, 15),
        ]
        self.awake_s = [
            pygame.transform.scale(
                sprite, (sprite.get_width() * 4,
                         sprite.get_height() * 4)) for sprite in awake
        ]
        self.link = self.sleep_s[0]

        zzz = [
            self.bubble_sheet.get_sprite(0, 0, 16, 15),
            self.bubble_sheet.get_sprite(17, 0, 16, 16),
        ]

        self.zzz_s = [
            pygame.transform.scale(
                sprite, (sprite.get_width() * 4,
                         sprite.get_height() * 4)) for sprite in zzz
        ]

        wake = [
            self.bubble_sheet.get_sprite(34, 0, 16, 15),
        ]
        wake_s = [
            pygame.transform.scale(
                sprite, (sprite.get_width() * 4,
                         sprite.get_height() * 4)) for sprite in wake
        ]

        self.animations: dict[str, list[pygame.Surface]] = {
            'link_sleep': self.sleep_s,
            'link_resleep': self.sleep_s,
            'link_awake': self.awake_s,
            'bubble_s': self.zzz_s,
            'bubble_w': wake_s,
        }
        self.current_frame = 0.0
        self.animation_speed = 1.0

        self.timeline: dict[str, TimelineStep] = {
            'fade_in': {'duration': 5.0, 'next': 'link_sleep'},
            'link_sleep': {'duration': 3.0, 'next': 'link_awake'},
            'link_awake': {'duration': 4.0, 'next': 'link_resleep'},
            'link_resleep': {'duration': 5.0, 'next': 'fade_out'},
            'fade_out': {'duration': 3.0, 'next': 'profile_name'},
        }
        self.phase_timer = 0.0
        self.fade_alpha = 0.0

        sheet_skins = SpriteSheet(
            resource_path("ui", "scores_skins.png"), game)
        self.username = ""
        sheet_skins_a = [
            sheet_skins.get_sprite(0, 0, 16, 14),
            sheet_skins.get_sprite(0, 13, 16, 14),
            sheet_skins.get_sprite(0, 26, 16, 14),
            sheet_skins.get_sprite(0, 39, 16, 14),
            sheet_skins.get_sprite(0, 52, 16, 14)
        ]
        self.sheet_skins = {
            "green": pygame.transform.scale(sheet_skins_a[0], (32, 32)),
            "dark": pygame.transform.scale(sheet_skins_a[1], (32, 32)),
            "purple": pygame.transform.scale(sheet_skins_a[2], (32, 32)),
            "blue": pygame.transform.scale(sheet_skins_a[3], (32, 32)),
            "red": pygame.transform.scale(sheet_skins_a[4], (32, 32))
        }
        self.skin_options = [
            "green",
            "dark",
            "purple",
            "blue",
            "red"
        ]
        self.skin_index = 0
        self.skin_color = "green"
        self.score_saved = False
        self.question_played = False

        self.bgm_loop = resource_path("music", "credits_zelda.mp3")

    def on_enter(self) -> None:
        """Prepare the scene when it becomes active."""
        self.play_theme(pygame.USEREVENT)

    def handle_event(self, event: pygame.event.Event) -> None:
        """Handle one pygame event for the scene.

        Args:
            event: Event data to handle.
        """
        if event.type == pygame.KEYDOWN:
            if self.phase == "win_screen":
                if event.key == pygame.K_RETURN:
                    self.game.sfx['press_menu'].play()
                self.phase = "fade_in"
            if self.phase == "menu":
                if event.key == pygame.K_UP:
                    self.game.sfx['move_menu'].play()
                    if self.state == "Replay":
                        self.state = "main_menu"
                    elif self.state == "main_menu":
                        self.state = "Replay"
                elif event.key == pygame.K_DOWN:
                    self.game.sfx['move_menu'].play()
                    if self.state == "main_menu":
                        self.state = "Replay"
                    elif self.state == "Replay":
                        self.state = "main_menu"
                if event.key == pygame.K_RETURN:
                    self.game.sfx['press_menu'].play()
                    if self.state == "Replay":
                        from pacman.game.scenes import PlayScene
                        self.game.scene_manager.change(PlayScene(self.game))
                    if self.state == "main_menu":
                        from pacman.game.scenes import MainMenuScene as Menu
                        self.game.scene_manager.change(Menu(self.game))

            elif self.phase == "profile_name":
                if not hasattr(event, "unicode"):
                    return
                if event.key == pygame.K_BACKSPACE:
                    self.username = self.username[:-1]
                elif event.key == pygame.K_RETURN or\
                        event.key == pygame.K_KP_ENTER:
                    if 0 < len(self.username) <= 10:
                        self.game.sfx['press_menu'].play()
                        self.phase = "profile_skin"
                else:
                    if len(self.username) < 10\
                            and (event.unicode.isalnum()
                                 or event.unicode.isspace()):
                        self.username += event.unicode
            elif self.phase == "profile_skin":
                if event.key == pygame.K_LEFT:
                    if self.skin_color == "green":
                        self.skin_color = "red"
                    elif self.skin_color == "dark":
                        self.skin_color = "green"
                    elif self.skin_color == "purple":
                        self.skin_color = "dark"
                    elif self.skin_color == "blue":
                        self.skin_color = "purple"
                    elif self.skin_color == "red":
                        self.skin_color = "blue"
                elif event.key == pygame.K_RIGHT:
                    if self.skin_color == "green":
                        self.skin_color = "dark"
                    elif self.skin_color == "dark":
                        self.skin_color = "purple"
                    elif self.skin_color == "purple":
                        self.skin_color = "blue"
                    elif self.skin_color == "blue":
                        self.skin_color = "red"
                    elif self.skin_color == "red":
                        self.skin_color = "green"
                elif event.key == pygame.K_RETURN:
                    self.phase = "menu"

    def update(self, dt: float) -> None:
        """Advance state for one frame.

        Args:
            dt: Elapsed frame time in milliseconds.
        """
        self.time += dt
        if self.time >= self.frame_delay:
            self.time = 0.0
            self.frame_index = (self.frame_index + 1) % len(self.link_anim)

        delta = dt / 1000.0

        anim = 'link_resleep' if self.phase == 'fade_out' else self.phase

        if anim in self.animations:
            current_animation = self.animations[anim]
            self.current_frame += self.animation_speed * delta
            if self.current_frame >= len(current_animation):
                self.current_frame = 0.0

        if self.phase in self.timeline:
            self.phase_timer += delta

            if self.phase == 'fade_out':
                duration = self.timeline['fade_out']['duration']
                progress = min(1.0, self.phase_timer / duration)
                self.fade_alpha = progress * 255

            elif self.phase == 'fade_in':
                self.fade_alpha = max(
                    0.0, 255 - (
                        self.phase_timer / self.timeline['fade_in']['duration']
                    ) * 255)

            if self.phase_timer >= self.timeline[self.phase]['duration']:
                self.phase = self.timeline[self.phase]['next']
                self.phase_timer = 0.0
                self.current_frame = 0.0

        if self.phase != 'win_screen':
            pygame.mixer.music.stop()

    def render(self, surface: pygame.Surface) -> None:
        """Draw the current state onto a surface.

        Args:
            surface: Surface that receives drawing operations.
        """
        surface.fill((0, 0, 0))

        if self.phase == 'win_screen':
            self.thanks_zelda(surface)

        if self.phase not in ['win_screen',
                              'profile_name',
                              'profile_skin',
                              'menu']:
            surface.blit(self.house, (0, 100))
            self.link_animation(surface)

        if self.phase in {'fade_out', 'end', 'fade_in'}:
            fade_surface = pygame.Surface(
                (self.game.logic_width, self.game.logic_height),
                pygame.SRCALPHA
            )
            fade_surface.fill((0, 0, 0, int(self.fade_alpha)))
            surface.blit(fade_surface, (0, 0))

        self.to_game(surface)
        self.to_main_menu(surface)
        self.save_score(surface)

    def thanks_zelda(self, surface: pygame.Surface) -> None:
        """Draw the Zelda thank-you sequence.

        Args:
            surface: Surface that receives drawing operations.
        """
        surface.fill((0, 0, 0))
        font = StringFont(surface,
                          resource_path("fonts", "zelda-gba.ttf"), 40)

        len_s = font.get_width("SCORE: " + str(self.game.in_game_score)) / 2
        font.write(
            "SCORE: " + str(self.game.in_game_score),
            surface.get_width() // 2 - len_s,
            30,
            (255, 255, 255)
        )

        len_s = font.get_width(
            "ITEMS COLLECTED: " + str(self.game.collected_items)) / 2
        items = len(self.game.collected_items)
        font.write(
            "ITEMS COLLECTED: " + str(items),
            surface.get_width() // 2 - len_s,
            80,
            (255, 255, 255)
        )

        len_s = font.get_width("THANKS ZELDA, YOU'RE") / 2
        font.write(
            "THANKS ZELDA, YOU'RE",
            surface.get_width() // 2 - len_s,
            surface.get_height() // 2 - 20,
            (255, 255, 255)
        )
        len_s = font.get_width("THE HERO OF HYRULE.") / 2
        font.write(
            "THE HERO OF HYRULE.",
            surface.get_width() // 2 - len_s,
            surface.get_height() // 2 + 20,
            (255, 255, 255)
        )
        # put here sprites of Zelda and Link (+60 height)
        surface.blit(self.link_anim[self.frame_index],
                     (surface.get_width() // 2 - 100,
                      surface.get_height() // 2 + 83))
        surface.blit(self.zelda,
                     (surface.get_width() // 2,
                      surface.get_height() // 2 + 82))

        len_s = font.get_width("FINALLY,") / 2
        font.write(
            "FINALLY,",
            surface.get_width() // 2 - len_s,
            surface.get_height() // 2 + 160,
            (255, 255, 255)
        )

        len_s = font.get_width("PEACE RETURNS TO HYRULE.") / 2
        font.write(
            "PEACE RETURNS TO HYRULE.",
            surface.get_width() // 2 - len_s,
            surface.get_height() // 2 + 200,
            (255, 255, 255)
        )

        len_s = font.get_width("THIS ENDS THE STORY.") / 2
        font.write(
            "THIS ENDS THE STORY.",
            surface.get_width() // 2 - len_s,
            surface.get_height() // 2 + 300,
            (255, 255, 255)
        )

    def save_score(self, surface: pygame.Surface) -> None:
        """Persist the current run score.

        Args:
            surface: Surface that receives drawing operations.
        """
        if self.phase == "profile_name":
            font = StringFont(surface,
                              resource_path("font", "zelda-gba.ttf"), 30)
            prompt = "Enter Name:" + self.username
            font.write(prompt,
                       surface.get_width() // 2 - font.get_width(prompt) // 2,
                       surface.get_height() // 3, (255, 255, 255))
        if self.phase == "profile_skin":
            font = StringFont(surface,
                              resource_path("font", "zelda-gba.ttf"), 30)
            prompt = "Choose Skin (Left/Right)"
            font.write(prompt,
                       surface.get_width() // 2 - font.get_width(prompt) // 2,
                       surface.get_height() // 3, (255, 255, 255))
            skin_name = self.skin_color
            skin_image = self.sheet_skins[skin_name]
            surface.blit(skin_image, (
                surface.get_width() // 2 - skin_image.get_width() // 2,
                surface.get_height() // 3 + 20))
        if self.phase == "menu" and not self.score_saved:
            highscores = load_highscores()
            highscores = insert_highscore(
                highscores, self.username,
                self.game.in_game_score,
                self.skin_color,
                self.game.cheat
            )
            save_highscores(highscores)
            self.score_saved = True

    def to_main_menu(self, surface: pygame.Surface) -> None:
        """Return to the main menu scene.

        Args:
            surface: Surface that receives drawing operations.
        """
        if self.phase == "menu":
            font = StringFont(surface,
                              resource_path("font", "zelda-gba.ttf"), 30)
            len_s = font.get_width("Quit")
            if self.state == "main_menu":
                font.write("> Quit", surface.get_width() // 2 - len_s // 2,
                           surface.get_height() // 3, (255, 255, 255))
            else:
                font.write("Quit", surface.get_width() // 2 - len_s // 2,
                           surface.get_height() // 3, (100, 100, 100))

    def to_game(self, surface: pygame.Surface) -> None:
        """Start or restart gameplay.

        Args:
            surface: Surface that receives drawing operations.
        """
        if self.phase == "menu":
            font = StringFont(surface,
                              resource_path("font", "zelda-gba.ttf"), 30)
            len_s = font.get_width("Replay")
            if self.state == "Replay":
                font.write("> Replay", surface.get_width() // 2 - len_s // 2,
                           surface.get_height() // 3 + 30, (255, 255, 255))
            else:
                font.write("Replay", surface.get_width() // 2 - len_s // 2,
                           surface.get_height() // 3 + 30, (100, 100, 100))

    def link_animation(self, surface: pygame.Surface) -> None:
        """Draw Link animation frames for the ending.

        Args:
            surface: Surface that receives drawing operations.
        """
        anim_key = (
            'link_resleep'
            if self.phase in {'fade_out', 'fade_in'}
            else self.phase
        )

        if anim_key not in self.animations:
            return
        anim = self.animations[anim_key]

        current_sprite = anim[int(self.current_frame)]

        if self.phase in {'link_sleep', 'link_resleep', 'fade_out', 'fade_in'}:
            surface.blit(current_sprite, (471, 222))
            surface.blit(
                self.animations['bubble_s'][int(self.current_frame)],
                (416, 195)
            )
        if self.phase == 'link_awake':
            if not self.question_played:
                self.game.sfx['question'].play()
                self.game.sfx['question'].set_volume(0.2)
                self.question_played = True
            surface.blit(current_sprite, (471, 222))
            surface.blit(
                self.animations['bubble_w'][int(self.current_frame)],
                (520, 180)
            )
